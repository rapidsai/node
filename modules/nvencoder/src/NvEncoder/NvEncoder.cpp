...
void NvEncoder::CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS *pIntializeParams,
  ...
  if (!m_bMotionEstimationOnly) {
    ...
    m_nvenc.nvEncGetEncodePresetConfigEx(
      m_hEncoder, codecGuid, presetGuid, tuningInfo, &presetConfig);
    memcpy(pIntializeParams->encodeConfig, &presetConfig.presetCfg, sizeof(NV_ENC_CONFIG));
  } else {
...
void NvEncoder::CreateEncoder(const NV_ENC_INITIALIZE_PARAMS *pEncoderParams) {
  ...
  if (pEncoderParams->encodeConfig) {
    ...
    NV_ENC_PRESET_CONFIG presetConfig = {NV_ENC_PRESET_CONFIG_VER, {NV_ENC_CONFIG_VER}};
    if (!m_bMotionEstimationOnly) {
      m_nvenc.nvEncGetEncodePresetConfigEx(m_hEncoder,
                                           pEncoderParams->encodeGUID,
                                           pEncoderParams->presetGUID,
                                           pEncoderParams->tuningInfo,
                                           &presetConfig);
  ...
  if (!m_bOutputInVideoMemory) { m_vpCompletionEvent.resize(m_nEncoderBuffer, nullptr); }
  ...
  if (m_bMotionEstimationOnly) {
    ...
    if (!m_bOutputInVideoMemory) { InitializeMVOutputBuffer(); }
  } else {
    if (!m_bOutputInVideoMemory) {
...
NVENCSTATUS NvEncoder::DoEncode(NV_ENC_INPUT_PTR inputBuffer,
  ...
  if (pPicParams) { picParams = *pPicParams; }
...
void NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer,
  ...
  for (; m_iGot < iEnd; m_iGot++) {
    ...
    if (vPacket.size() < i + 1) { vPacket.push_back(std::vector<uint8_t>()); }
    vPacket[i].clear();
...
bool NvEncoder::Reconfigure(const NV_ENC_RECONFIGURE_PARAMS *pReconfigureParams) {
  ...
  memcpy(
    &m_initializeParams, &(pReconfigureParams->reInitEncodeParams), sizeof(m_initializeParams));
  if (pReconfigureParams->reInitEncodeParams.encodeConfig) {
    memcpy(
      &m_encodeConfig, pReconfigureParams->reInitEncodeParams.encodeConfig, sizeof(m_encodeConfig));
  }
...
NV_ENC_REGISTERED_PTR NvEncoder::RegisterResource(void *pBuffer,
...
void NvEncoder::WaitForCompletionEvent(int iEvent) {
  ...
  if (0U == sInitializeParams.enableEncodeAsync) { return; }
#ifdef DEBUG
...
uint32_t NvEncoder::GetNumChromaPlanes(const NV_ENC_BUFFER_FORMAT bufferFormat) {
  switch (bufferFormat) {
    case NV_ENC_BUFFER_FORMAT_NV12:
    case NV_ENC_BUFFER_FORMAT_YUV420_10BIT: return 1;
    case NV_ENC_BUFFER_FORMAT_YV12:
    case NV_ENC_BUFFER_FORMAT_IYUV:
...
void NvEncoder::GetChromaSubPlaneOffsets(const NV_ENC_BUFFER_FORMAT bufferFormat,
  ...
  switch (bufferFormat) {
    case NV_ENC_BUFFER_FORMAT_NV12:
    case NV_ENC_BUFFER_FORMAT_YUV420_10BIT: chromaOffsets.push_back(pitch * height); return;
    case NV_ENC_BUFFER_FORMAT_YV12:
    case NV_ENC_BUFFER_FORMAT_IYUV:
...
void NvEncoder::GetInitializeParams(NV_ENC_INITIALIZE_PARAMS *pInitializeParams) {
  if (!pInitializeParams || !pInitializeParams->encodeConfig) {
    NVENC_THROW_ERROR("Both pInitializeParams and pInitializeParams->encodeConfig can't be NULL",
                      NV_ENC_ERR_INVALID_PTR);
  }
...
NVENCSTATUS NvEncoder::DoMotionEstimation(NV_ENC_INPUT_PTR inputBuffer,
