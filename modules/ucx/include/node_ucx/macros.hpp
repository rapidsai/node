...
#define CUDA_THROW(e, c) NAPI_THROW(node_ucx::cudaError(e, c, __FILE__, __LINE__), (e).Undefined())
...
#define CUDA_THROW_ASYNC(task, status)                                                    \
  (task)->Reject(node_ucx::cudaError((task)->Env(), status, __FILE__, __LINE__).Value()); \
  return (task)->Promise()
