...
function getKey({ state, type, id }) { return `${state}-${type}-${id}`; }

export const COLOR_SCALE = scaleThreshold().domain([0, 4, 8, 12, 20, 32, 52, 84, 136, 220]).range([
  [26, 152, 80],
  [102, 189, 99],
  [166, 217, 106],
  [217, 239, 139],
...
export default class App extends Component {
  constructor(props) {
    ...
    this.state = { hoveredObject: null, ...this._aggregateAccidents(props.accidents) };
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.accidents !== this.props.accidents) {
      this.setState({ ...this._aggregateAccidents(nextProps.accidents) });
    }
  }
  ...
  _getLineColor(f, fatalities) {
    if (!fatalities) { return [200, 200, 200]; }
