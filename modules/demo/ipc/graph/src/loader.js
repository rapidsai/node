...
async function request(sock, req = 'ready') {
  const resp = await sock.send(req).then(() => sock.receive()).then((resp) => `${resp}`);
  switch (resp) {
    case '':
    case 'close': return resp;
...
function openMemHandle(handle) { return new Uint8Buffer(new IpcMemory(handle)); }
