...
describeWithFlags('complex64 memory', ALL_ENVS, () => {
    it('usage', async () => {
        const webglSizeUploadUniformFlagSaved = tf.env().get('WEBGL_SIZE_UPLOAD_UNIFORM');
    ...
    it('Multiple complex tensors sharing same underlying components works', async () => {
        const numTensors = tf.memory().numTensors;
        const numDataIds = tf.engine().backend.numDataIds();
...
//# sourceMappingURL=Complex_test.js.map