...
import { getBinaryCache, MathBackendWebGL } from '@tensorflow/tfjs-backend-webgl/dist/backend_webgl';
import { computeBytes } from '@tensorflow/tfjs-backend-webgl/dist/texture_manager';
import { PhysicalTextureType } from '@tensorflow/tfjs-backend-webgl/dist/tex_util';
...
//     let webglLazilyUnpackFlagSaved;
//     let webglCpuForwardFlagSaved;
//     beforeAll(() => {
...
//         expectArraysEqual(decodeStrings(backend.readSync(t.dataId)), ['a', 'b', 'c']);
//     });
...
//         const webglSizeUploadUniformSaved = tf.env().getNumber('WEBGL_SIZE_UPLOAD_UNIFORM');
...
describeWithFlags('upload tensors as uniforms', FLOAT32_WEBGL_ENVS, () => {
    ...
    it('download and re-upload an output of a shader', () => {
        ...
        expected.fill(16);
        expectArraysClose(res.dataSync(), expected);
...
//         const shapeRC = [2, 3];
//         let bytesForTex = computeBytes(shapeRC, PhysicalTextureType.UNPACKED_FLOAT16, gpgpu.gl, gpgpu.textureConfig, false /* isPacked */);
//         expect(bytesForTex).toBe(96);
//         bytesForTex = computeBytes(shapeRC, PhysicalTextureType.UNPACKED_FLOAT32, gpgpu.gl, gpgpu.textureConfig, false /* isPacked */);
//         expect(bytesForTex).toBe(96);
//         bytesForTex = computeBytes(shapeRC, PhysicalTextureType.PACKED_4X1_UNSIGNED_BYTE, gpgpu.gl, gpgpu.textureConfig, true /* isPacked */);
//         expect(bytesForTex).toBe(32);
//         bytesForTex = computeBytes(shapeRC, PhysicalTextureType.PACKED_2X2_FLOAT32, gpgpu.gl, gpgpu.textureConfig, true /* isPacked */);
//         expect(bytesForTex).toBe(32);
//         bytesForTex = computeBytes(shapeRC, PhysicalTextureType.PACKED_2X2_FLOAT16, gpgpu.gl, gpgpu.textureConfig, true /* isPacked */);
//         expect(bytesForTex).toBe(32);
//         gpgpu.dispose();
//     });
// });
...
//     it('basic', () => {
//         const savedDeleteThreshold = tf.env().get('WEBGL_DELETE_TEXTURE_THRESHOLD');
//         tf.env().set('WEBGL_DELETE_TEXTURE_THRESHOLD', 0);
//         const a = tf.tensor2d([1, 2, 3, 4, 5, 6], [2, 3]);
//         const b = tf.tensor2d([0, 1, -3, 2, 2, 1], [3, 2]);
//         tf.matMul(a, b);
//         const startNumBytesAllocated = tf.memory().numBytesInGPUAllocated;
//         a.dispose();
//         b.dispose();
//         expect(startNumBytesAllocated -
//             tf.memory().numBytesInGPUAllocated)
//             .toBeGreaterThan(0);
...
//         tf.registerBackend(customWebGLBackendName, () => new MathBackendWebGL(null));
//     });
//     afterEach(() => {
...
//         const a = tf.zeros([10, 10]);
//         const time = await tf.time(() => a.square().dataSync());
//         expect(time.uploadWaitMs > 0);
//         expect(time.downloadWaitMs > 0);
//         expect(time.kernelMs > 0);
//         expect(time.wallMs >= time.kernelMs);
...
//         const a = tf.zeros([10, 10]);
//         const time = await tf.time(async () => a.square().data());
//         expect(time.uploadWaitMs > 0);
//         expect(time.downloadWaitMs > 0);
//         expect(time.kernelMs > 0);
//         expect(time.wallMs >= time.kernelMs);
...
//         // Pre-upload a on gpu.
//         a.square();
//         const time = await tf.time(() => a.sqrt());
...
//         expect(time.wallMs >= time.kernelMs);
//     });
//     it('returns error for kernelMs if query timer extension is unavailable', async () => {
//         const savedQueryReliableValue = tf.env().get('WEBGL_DISJOINT_QUERY_TIMER_EXTENSION_RELIABLE');
//         tf.env().set('WEBGL_DISJOINT_QUERY_TIMER_EXTENSION_RELIABLE', false);
//         const a = tf.zeros([10, 10]);
//         const time = await tf.backend().time(() => a.sqrt());
