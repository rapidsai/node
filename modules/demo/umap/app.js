...
async function localRenderLoop() {
    await Promise.resolve().then(() => {
        ...
        numNodes = nodeUpdates[0].length = nodeUpdates[0].position.length;
...
async function remoteRenderLoop() {
    ...
    for await (const msg of ipchs) {
        ...
        // console.log(nodeTokens);
        // let nodeIPCBuffers = loadIpcHandles(nodeTokens, ['size', 'color', 'position']);
        let nodeIPCBuffers = loadIpcHandles(nodeTokens);
