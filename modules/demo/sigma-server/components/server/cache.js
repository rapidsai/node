...
const {DataFrame, Series, Int32, Uint32, Int16, Int8, Float32, StringSeries} = require('@rapidsai/cudf');
...
async function readDataset(datasets, datasetName) {
  ...
  if (datasetName == 'sigma_demo') {
    datasets.sigma_demo = null;
    return readSigmaDemo();
  }
}
...
readSigmaDemo() {
  ...
  const trips = DataFrame.readCSV({
    header: 0,
    sourceType: 'files',
    sources: [path.resolve('./public', 'data/sigma_demo.csv')],
    dataTypes: {
      sourceid: new Int16,
      dstid: new Int16,
      month: new Int8,
  ...
  */
  return new DataFrame({
    'key': StringSeries.new(["bob", "joe", "mary"]),
    'label': StringSeries.new(["bob", "joe", "mary"]), 
    'tag': StringSeries.new(["bob", "joe", "mary"]), 
    'URL': StringSeries.new(["bob", "joe", "mary"]), 
    'cluster': Series.new(new Int32Array([1, 1, 2])), 
    'x': Series.new(new Int32Array([1, 1, 2])), 
    'y': Series.new(new Int32Array([1, 1, 2])), 
    'score': Series.new(new Int32Array([1, 1, 2])), 
  });
}

async function
readUberTrips() {
  console.log(path.resolve('./public', 'data/san_fran_uber.csv'));
  const trips = DataFrame.readCSV({
    header: 0,
    sourceType: 'files',
