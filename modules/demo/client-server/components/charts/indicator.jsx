...
export default class Indicator extends React.Component {
  constructor(props) {
    super(props);
    this.state = { current_query: this.props.getquery(), size: 0 }
  }
  ...
  async _updateSize() {
    ...
    if (!url) { return null; }
    const size =
      await fetch(url, { method: 'GET' }).then((res) => res.json()).then(data => { return data });
    return size;
  }
  ...
  render() {
    return (
      <Card border='light' bg='secondary' text='light' className='text-center mb-3'>
        <Card.Header className='h5'>Data Points Selected</Card.Header>
        <Card.Body>
