...
struct FromJS {
  ...
  // Objects
  //
  inline operator CUDARTAPI::cudaDeviceProp() const {
    ...
    if (val.IsObject()) {
      ...
      visit_struct::for_each(props, [&](const char* key, auto& val) {
        ...
        if (!std::is_pointer<T>()) {
          ...
          *dst   = FromJS(obj.Get(key)).operator T();
        } else {
          using T_ = typename std::remove_pointer<T>::type;
          T_* src  = reinterpret_cast<T_*>(FromJS(obj.Get(key)).operator T());
...
struct ToNapi {
  ...
  Napi::Object inline operator()(const CUDARTAPI::cudaDeviceProp& props) const {
    auto cast_t = *this;
    auto obj    = Napi::Object::New(this->env);
