...
function bufferData(
  ...
  usage: GLenum,
  srcOffset: GLuint,
  srcByteLength?: GLuint,
): void;
function bufferData(
  this: WebGL2RenderingContext,
  ...args: [GLenum, GLsizeiptr | BufferSource | null, GLenum, GLuint?, GLuint?]
): void {
  let [target, src, usage, srcOffset, srcByteLength] = args;
  if (args.length > 3 && src !== null && typeof src !== 'number' && typeof srcOffset === 'number') {
    let BPM;
    const arr = ArrayBuffer.isView(src) ? src : new Uint8Array(src);
...
function bufferSubData(
  this: WebGL2RenderingContext,
  ...args: [GLenum, GLintptr, BufferSource | ArrayBufferView, GLuint?, GLuint?]
): void {
...
function getShaderInfoLog(this: WebGL2RenderingContext, shader: WebGLShader): string {
  ...
  return lines
    .map((line) => {
      ...
      ({ index: errIndex, [0]: errMatch, [1]: type } = (lines[0] || '').match(
        /\s?(warning|error) (\w+|\d+):/,
      ) || { ...['', ''] });
      // eslint-disable-next-line @typescript-eslint/prefer-regexp-exec
      ...
      if (errIndex !== undefined && numIndex !== undefined && errMatch && type && numMatch && num) {
        return `${type.toUpperCase()}:${line.slice(errIndex + errMatch.length)}:${num}${line.slice(
          numIndex + numMatch.length,
        )}`;
      }
      return line;
    })
...
function texImage2D(
  ...
  switch (args.length) {
    case 6: {
      [target, level, internalformat, format, type, src] = args as [
        GLenum,
        GLint,
        GLint,
        GLenum,
    ...
    case 10: {
      if (typeof args[8] === 'number') {
        [target, level, internalformat, width, height, border, format, type, src] = args as [
          GLenum,
          GLint,
          GLint,
          GLsizei,
    ...
    default:
      throw new TypeError('WebGLRenderingContext texImage2D() takes 6, 9, or 10 arguments');
  }
...
function texSubImage2D(
  ...
  switch (args.length) {
    case 7: {
      [target, level, x, y, format, type, src] = args as [
        GLenum,
        GLint,
        GLint,
        GLint,
    ...
    case 10: {
      if (typeof args[8] === 'number') {
        [target, level, x, y, width, height, format, type, src] = args as [
          GLenum,
          GLint,
          GLint,
          GLint,
      ...
      if (
        ...
        src = toArrayBufferViewSlice(<ArrayBufferView>src, offset);
        break;
      }
      if (args[8] && typeof args[8] === 'object') {
        [target, level, x, y, width, height, format, type, src] = args as [
  ...
  switch (args.length) {
    ...
    case 7: {
      if (typeof args[6] === 'number') {
        [x, y, width, height, format, type, dst] = args as [
          GLint,
          GLint,
          GLsizei,
          GLsizei,
      ...
      if (
        ...
        dst = toArrayBufferViewSlice(<ArrayBufferView>dst, offset);
        break;
      }
      throw new TypeError('WebGLRenderingContext readPixels() invalid readPixels target');
    }
    case 8: {
      [x, y, width, height, format, type, dst, offset = 0] = args as [
        GLint,
        GLint,
        GLsizei,
        GLsizei,
    ...
    default:
      throw new TypeError('WebGLRenderingContext readPixels() takes 6, 7, or 8 arguments');
  }
...
function wrapAndLogGLMethods(proto: any) {
  ...
  const listToString = (x: any) => {
    ...
    return `(length=${x.length}, values=[${x.slice(0, 3)}, ... ${x.slice(
      (x.length >> 1) - 2,
      (x.length >> 1) + 1,
    ...
    switch (typeof x) {
      ...
      default:
        ...
        if (ArrayBuffer.isView(x)) {
          return `${x.constructor.name}${listToString(x)}`;
        }
        if (nodeCustomInspectSym in x) {
          return x[nodeCustomInspectSym]();
        }
        if (`${x}` === '[object Object]') {
  ...
  const logArguments = (name: string, fn: any) =>
...
function toArrayBufferViewSlice(source?: null, offset?: number): null;
function toArrayBufferViewSlice(source: ArrayBuffer | ArrayBufferView, offset?: number): Uint8Array;
function toArrayBufferViewSlice(
