...
export function keyboardEvents(window: GLFWDOMWindow) {
  ...
  const characterKeys = keys.pipe(filter(isCharacterKey), (charKeys) =>
    characterUpdates(window, charKeys),
  );
...
function characterUpdates(window: GLFWDOMWindow, charKeys: Observable<GLFWKeyboardEvent>) {
  const charCodes = windowCallbackAsObservable(glfw.setCharCallback, window).pipe(
  ...
  return charCodes
    .pipe(
      withLatestFrom(charKeys, (charCode, keyEvt) => {
...
function isCharacterKey(evt: GLFWKeyboardEvent) {
  return !isSpecialKey(evt);
}
...
export class GLFWKeyboardEvent extends GLFWEvent {
  public static fromKeyEvent(
    ...
    const evt = new GLFWKeyboardEvent(
      action === glfw.PRESS ? 'keydown' : action === glfw.RELEASE ? 'keyup' : 'keypress',
    );
