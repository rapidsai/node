...
import { map, filter, publish, mergeAll, refCount, withLatestFrom } from 'rxjs/operators';
import { windowCallbackAsObservable, GLFWEvent, isAltKey, isCtrlKey, isMetaKey, isShiftKey, isCapsLock } from './event';
...
export function keyboardEvents(window: GLFWDOMWindow) {
    ...
    const characterKeys = keys.pipe(filter(isCharacterKey),
        (charKeys) => characterUpdates(window, charKeys));
...
function keyUpdates(window: GLFWDOMWindow) {
    return windowCallbackAsObservable(glfw.setKeyCallback, window)
        .pipe(map(([_, ...rest]) => GLFWKeyboardEvent.fromKeyEvent(window, ...rest)))
        .pipe(publish(), refCount())
}

function characterUpdates(window: GLFWDOMWindow, charKeys: Observable<GLFWKeyboardEvent>) {
    const charCodes =
        windowCallbackAsObservable(glfw.setCharCallback, window)
            .pipe(map(([_, charCode]) => charCode), publish(), refCount());
    return charCodes.pipe(withLatestFrom(charKeys, (charCode, keyEvt) => {
        if (keyEvt.type === 'keydown') {
            return [
                Object.assign(keyEvt.asCharacter(       0, keyEvt.target.modifiers), { type: 'keydown' }),
                Object.assign(keyEvt.asCharacter(charCode, keyEvt.target.modifiers), { type: 'keypress' }),
            ]
        }
...
export class GLFWKeyboardEvent extends GLFWEvent {
    ...
    public static fromKeyEvent(window: GLFWDOMWindow, key: number, scancode: number, action: number, modifiers: number) {

        const evt = new GLFWKeyboardEvent(
            action === glfw.PRESS ? 'keydown' :
            action === glfw.RELEASE ? 'keyup' : 'keypress');
        ...
        const isControl = !isSuper && (key === GLFWKey.KEY_LEFT_CONTROL || key === GLFWKey.KEY_RIGHT_CONTROL);
    ...
    public _rawKey: number = 0;
    public _rawScanCode: number = 0;
