...
export class GLFWEvent {
  ...
  public get srcElement() {
    return this.target;
  }
  public get relatedTarget() {
    return null;
  }
...
type GLFWCallbackArgs<T extends SetGLFWCallback> = T extends (cb: (...args: infer P) => void) => any
  ? P
  : never;

type SetWindowCallback = (ptr: number, cb: null | ((...args: any) => void)) => void;
type WindowCallbackArgs<T extends SetWindowCallback> = T extends (
  ptr: number,
...
export function windowCallbackAsObservable<C extends SetWindowCallback>(
  ...
  return new Observable<Args>((observer: Observer<Args>) => {
    ...
    return trySetCallback(setCallback.name, () => setCallback(window.id, next))
      ? dispose
      : () => {};
  });
}
