...
import { windowCallbackAsObservable, GLFWEvent } from './event';
...
function framebufferSizeUpdates(window: GLFWDOMWindow) {
    return windowCallbackAsObservable(glfw.setFramebufferSizeCallback, window)
        .pipe(map(([_, ...rest]) => GLFWWindowEvent.fromFramebufferResize(window, ...rest)))
        .pipe(publish(), refCount());
}

function closeUpdates(window: GLFWDOMWindow) {
    return windowCallbackAsObservable(glfw.setWindowCloseCallback, window)
        .pipe(map(([_, ...rest]) => GLFWWindowEvent.fromClose(window, ...rest)))
        .pipe(publish(), refCount());
}
...
export class GLFWWindowEvent extends GLFWEvent {
    ...
    public static fromFramebufferResize(window: GLFWDOMWindow, frameBufferWidth: number, frameBufferHeight: number) {
    ...
    public static fromClose(window: GLFWDOMWindow) {
        return GLFWWindowEvent.create(window, 'close');
    }
    ...
    public static fromFocus(window: GLFWDOMWindow, focused: boolean) {
        return Object.assign(GLFWWindowEvent.create(window, focused ? 'focus' : 'blur'), { _focused: !!focused });
    }

    public static fromIconify(window: GLFWDOMWindow, minimized: boolean) {
        return Object.assign(GLFWWindowEvent.create(window, minimized ? 'minimized' : 'restored'), { _minimized: !!minimized });
    }

    public static fromMaximize(window: GLFWDOMWindow, maximized: boolean) {
        return Object.assign(GLFWWindowEvent.create(window, maximized ? 'maximized' : 'restored'), { _maximized: !!maximized });
    }
    ...
    private _frameBufferWidth: number = 0;
    private _frameBufferHeight: number = 0;
