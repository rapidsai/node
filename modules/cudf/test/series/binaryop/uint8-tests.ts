...
describe('Series binaryops (Uint8)', () => {
  describe('Series.add', () => {
    test('adds a Series', () => {
  ...
  describe('Series.sub', () => {
    test('subtracts a Series', () => {
      ...
      // lhs - rhs == [0 - 1, 1 - 2, 2 - 3]
      expect([...lhs.sub(rhs)].map(Number)).toEqual([255, 255, 255]);
      // rhs - lhs == [1 - 0, 2 - 1, 3 - 2]
  ...
  describe('Series.div', () => {
    test('divides by a Series', () => {
      const {lhs, rhs} = makeTestData();
      // lhs / lhs == [0/0, 1/1, 2/2]
      expect([...lhs.div(lhs)].map(Number)).toEqual([255, 1, 1]);
      // lhs / rhs == [0/1, 1/2, 2/3]
  ...
  describe('Series.mod', () => {
    test('modulo by a Series', () => {
      const {lhs, rhs} = makeTestData();
      // lhs % lhs == [0 % 0, 1 % 1, 2 % 2])
      expect([...lhs.mod(lhs)].map(Number)).toEqual([255, 0, 0]);
      // lhs % rhs == [0 % 1, 1 % 2, 2 % 3])
  ...
  describe('Series.bitwise_or', () => {
    ...
    test('bitwise_or with a scalar', () => {
      const {lhs} = makeTestData();
      expect([...lhs.bitwise_or(-1)].map(Number)).toEqual([255, 255, 255]);
    ...
    test('bitwise_or with a bigint', () => {
      const {lhs} = makeTestData();
      expect([...lhs.bitwise_or(-1n)].map(Number)).toEqual([255, 255, 255]);
  ...
  describe('Series.bitwise_xor', () => {
    ...
    test('bitwise_xor with a scalar', () => {
      const {lhs} = makeTestData();
      expect([...lhs.bitwise_xor(-1)].map(Number)).toEqual([255, 254, 253]);
    ...
    test('bitwise_xor with a bigint', () => {
      const {lhs} = makeTestData();
      expect([...lhs.bitwise_xor(-1n)].map(Number)).toEqual([255, 254, 253]);
