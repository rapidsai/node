...
describe('Series binaryops (Uint16)', () => {
  describe('Series.add', () => {
    test('adds a Series', () => {
  ...
  describe('Series.floorDiv', () => {
    test('floorDivides by a Series', () => {
      ...
      expect([...lhs.floorDiv(lhs)].map(Number)).toEqual([65535, 1, 1]);
  ...
  describe('Series.bitwiseOr', () => {
    ...
    test('bitwiseOr with a scalar', () => {
      const {lhs} = makeTestData();
      expect([...lhs.bitwiseOr(-1)].map(Number)).toEqual([65535, 65535, 65535]);
    ...
    test('bitwiseOr with a bigint', () => {
      const {lhs} = makeTestData();
      expect([...lhs.bitwiseOr(-1n)].map(Number)).toEqual([65535, 65535, 65535]);
  ...
  describe('Series.bitwiseXor', () => {
    ...
    test('bitwiseXor with a scalar', () => {
      const {lhs} = makeTestData();
      expect([...lhs.bitwiseXor(-1)].map(Number)).toEqual([65535, 65534, 65533]);
    ...
    test('bitwiseXor with a bigint', () => {
      const {lhs} = makeTestData();
      expect([...lhs.bitwiseXor(-1n)].map(Number)).toEqual([65535, 65534, 65533]);
