...
const makeTestData = (values?: (number|null)[]) =>
  makeTestSeries(new arrow.Uint16, makeTestNumbers(values)).lhs;

describe('Series unaryops (Uint16)', () => {
  ...
  test('Series.sinh', () => { expect(runMathOp('sinh')).toEqual([65535, 0, 10]); });
  test('Series.cosh', () => { expect(runMathOp('cosh')).toEqual([65535, 1, 10]); });
  test('Series.tanh', () => { expect(runMathOp('tanh')).toEqual([1, 0, 0]); });
  test('Series.asinh', () => { expect(runMathOp('asinh')).toEqual([11, 0, 1]); });
  test('Series.acosh', () => { expect(runMathOp('acosh')).toEqual([11, 0, 1]); });
  test('Series.atanh', () => { expect(runMathOp('atanh')).toEqual([0, 0, 0]); });
  test('Series.exp', () => { expect(runMathOp('exp')).toEqual([65535, 1, 20]); });
  test('Series.log', () => { expect(runMathOp('log')).toEqual([11, 0, 1]); });
  test('Series.sqrt', () => { expect(runMathOp('sqrt')).toEqual([255, 0, 1]); });
  test('Series.cbrt', () => { expect(runMathOp('cbrt')).toEqual([40, 0, 1]); });
  test('Series.ceil', () => { expect(runMathOp('ceil')).toEqual([65533, 0, 3]); });
  test('Series.floor', () => { expect(runMathOp('floor')).toEqual([65533, 0, 3]); });
  test('Series.abs', () => { expect(runMathOp('abs')).toEqual([65533, 0, 3]); });
  test('Series.not', () => {
  ...
  testForEachNumericType(
    ...
    function testSeriesView<T extends TypedArray|BigIntArray, R extends Numeric>(
      ...
      const expected = new TypedArrayCtor(new Uint16Array(input).buffer);
      expect(actual.subarray(0, expected.length)).toEqualTypedArray(expected);
    });
});
