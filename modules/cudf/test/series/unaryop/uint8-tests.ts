...
const makeTestData = (values?: (number|null)[]) =>
  makeTestSeries(new arrow.Uint8, makeTestNumbers(values)).lhs;
...
describe('Series unaryops (Uint8)', () => {
  ...
  test('Series.bitInvert', () => {
    ...
    expect([...actual]).toEqual([null, 255, 252, 249]);
  });
  const clampValuesLikeUnaryCast = clampIntValuesLikeUnaryCast(new Uint8Array([0]));
  testForEachNumericType(
    'Series.cast %p',
  ...
  testForEachNumericType(
    ...
    function testSeriesView<T extends TypedArray|BigIntArray, R extends Numeric>(
      ...
      const expected = new TypedArrayCtor(new Uint8Array(input).buffer);
      expect(actual).toEqualTypedArray(expected);
    });
});
