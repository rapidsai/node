...
export class Table extends(<TableConstructor>CUDF.Table) {
  ...
  get columns(): ReadonlyArray<string> { return this._data.names; }
  ...
  slice(start: number|string, end: number|string): CUDFTable {
    return new Table({
      data: this._data.sliceByColumnIndices(this.transformInputLabel(start),
  ...
  drop(props: {columns: Array<string>}) {
    props.columns.forEach((value) => { this._data.removeByColumnName(value); })
    super.updateColumns({columns: this._data.columns});
  }
...
Object.setPrototypeOf(CUDF.Table.prototype, new Proxy({}, {
                        get(target: any, p: any, table: any) {
                          ...
                          switch (typeof p) {
                            // @ts-ignore
                            case 'string':
                              if (table.columns.includes(i)) { return table.getColumnByName(i); }
                              break;
                          }
