...
describe('CSV', () => {
  test('create, list, describe, and drop CSV table', async () => {
    ...
    const sqlContext = await SQLCluster.init({numWorkers: 2});
    await sqlContext.createCSVTable('test_table', [path]);
    ...
    await sqlContext.dropTable('test_table');
  ...
  test('query CSV table', async () => {
    ...
    await expect(sqlContext.sql('SELECT c FROM test_table')).resolves.toStrictEqual([new DataFrame(
      {'c': Series.new(['foo', 'bar', 'foo'])})]);
  });
});
...
describe('Parquet', () => {
  test('create, list, describe, and drop Parquet table', async () => {
    ...
    const sqlContext = await SQLCluster.init({numWorkers: 2});
    await sqlContext.createParquetTable('test_table', [path]);
  ...
  test('query Parquet table', async () => {
    ...
    await expect(sqlContext.sql('SELECT b FROM test_table')).resolves.toStrictEqual([new DataFrame(
      {'b': Series.new(['foo', 'bar', 'foo'])})]);
  });
});

describe('ORC', () => {
  test('create, list, describe, and drop ORC table', async () => {
    const a  = Series.new([1.0, 2.0, 3.0]);
    const b  = Series.new(['foo', 'bar', 'foo']);
    const df = new DataFrame({'a': a, 'b': b});
    ...
    const sqlContext = await SQLCluster.init({numWorkers: 2});
    await sqlContext.createORCTable('test_table', [path]);
